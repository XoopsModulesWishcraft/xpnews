...
$cache = "xpnews_index_array%%".$category."_".$group."_".$cursor;
# ---------------------------------------------------------------------
...
if (!$read = XoopsCache::read($cache)){
	...
	XoopsCache::write($cache, array('code' => $code, 'count' => $count, 'curr_category' => $curr_category, 'group'=>$group,
								...
								'highmark' => $highmark, 'lowmark' => $lowmark, 'show_from' => $show_from, 'page' => $page), 
...
for($i=1;$i<=$totalpg;$i++)
	if ($i == $page)
		$xoopsTpl->append("pages",array('value' => XOOPS_URL."/modules/xpnews/indexing.php??category=$curr_category&group=$group&page=$i",'text' => "Page $i of $totalpg", 'select' => 'selected'));
	else
