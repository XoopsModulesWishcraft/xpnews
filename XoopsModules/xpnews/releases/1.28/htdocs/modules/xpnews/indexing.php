...
$cache = "xpnews_index_array%%".$curr_category."_".$_REQUEST['group'].'_'.$news_server[$curr_category]."%%".$_GET['cursor'].'_'.$_GET['page'];
# ---------------------------------------------------------------------
...
if (!$read = XoopsCache::read($cache)){
	...
	if( $ncount == 0 ) {
		...
		foreach( $xover as $artnum => $ov ) {
			...
			if (trim($author)=='')
				$author = 'unknown';
			$cache_posts[]= array("class" => $class, "num" => $artidx, "subject" => $readlink, "author" => $author, "time" =>$datestr);
	...
	XoopsCache::write($cache, array('code' => $code, 'count' => $count, 'curr_category' => $curr_category, 'group'=>$_REQUEST['group'],
								'cursor' => $cursor, 'show_end' => $show_end, 'xover' => $xover, 'ncount' => $ncount, 
								'cache_posts' => $cache_posts, 'artsppg' => $artsppg, 'artlist' => $artlist, 'artsize' => $artsize, 
								'highmark' => $highmark, 'lowmark' => $lowmark, 'show_from' => $show_from, 'page' => $page, 'totalpg' => $totalpg), 
								$GLOBALS['xoopsModuleConfig']['hours_cache'] * (60*60));
} else {
	extract($read);
}	
...
if ($GLOBALS['xoopsModuleConfig']['htaccess']) {
	if ($cursor==0)
		$url = XOOPS_URL.'/indexing,'.$curr_category.','.(isset($group)?$group:$_REQUEST['group']).'.'.$GLOBALS['xoopsModuleConfig']['seo_filetype'].'?';
	else
		$url = XOOPS_URL.'/indexing,cursor,'.$curr_category.','.(isset($group)?$group:$_REQUEST['group']).','.$cursor.'.'.$GLOBALS['xoopsModuleConfig']['seo_filetype'].'?';
	if (strpos($url, urldecode($_SERVER['REQUEST_URI']))==0) {
...
$xoopsTpl->assign("posts", $cache_posts);	
$xoopsTpl->assign("newsgroup", $group);
$xoopsTpl->assign('xoops_pagetitle', $group .' :: ' .$lowmark .' < '.$highmark);
if( $ncount == 0 ) {
...
for($i=1;$i<=$totalpg;$i++)
	if ($i == $page)
		$xoopsTpl->append("pages",array('value' => XOOPS_URL."/modules/xpnews/indexing.php?category=$curr_category&group=$group&page=$i",'text' => "Page $i of $totalpg", 'select' => 'selected="selected"'));
	else
		$xoopsTpl->append("pages",array('value' => XOOPS_URL."/modules/xpnews/indexing.php?category=$curr_category&group=$group&page=$i", 'text' => "Page $i of $totalpg", 'select' => ''));
