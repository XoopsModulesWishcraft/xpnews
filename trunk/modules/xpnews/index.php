...
if ($GLOBALS['xoopsModuleConfig']['htaccess']) {
	$url = XOOPS_URL.'/list,'.$server_obj->name().','.($curr_category).'.'.$GLOBALS['xoopsModuleConfig']['seo_filetype'].'?';
...
if ((($xoopsModuleConfig['cache_index_last'] + ($xoopsModuleConfig['cache_index']*60))>time())||$newsgroup_handler->getCount($criteria)==0)
{
	...
	while ( list ($group, $value) = each ($active) ) {
		...
		$glink = "<a href=\"".XOOPS_URL."/modules/xpnews/indexing.php?category=$curr_category&group=$group\">$group</a>";
	...
	if (!empty($newsgroup))
		...
		{
			$class = ($class=="even")?"odd":"even";	
			$glink = "<a href=\"".XOOPS_URL."/modules/xpnews/indexing.php?category=".$news->getVar('server_id')."&group=".$news->getVar('newsgroup')."\">".$news->getVar('newsgroup')."</a>";
