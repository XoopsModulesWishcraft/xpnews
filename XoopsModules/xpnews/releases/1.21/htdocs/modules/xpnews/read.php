...
$cache = "xpnews_article_array%%".$curr_category."_".$group."_".$artnum.'_'.$server.'_'.$group_default_server.'%%'.$news_server[$curr_category];
...
if (!$read = XoopsCache::read($cache)){
	...
	XoopsCache::write($cache, array('message' => $message,'artnum' => $artnum, 'artinfo'=>$artinfo, 'show_mode' => $show_mode,
									'artconv' => $artconv, 'email' => $email, 'curr_category' => $curr_category, 'group'=>$group,
									'lasturl' => $lasturl, 'nexturl' => $nexturl, 'code' => $code, 'count' => $count, 
									'lowmark' => $lowmark, 'highmark' => $highmark, 'thlist' => $thlist, 'date' => $date, 
									'from' => $from, 'subject' => $subject, 'org' => $org, 'idxurl' => $idxurl), 
									$GLOBALS['xoopsModuleConfig']['days_cache'] * (24*(60*60)));
	$nnrp->close();
} else {
	extract($read);
}	
...
if ($GLOBALS['xoopsModuleConfig']['htaccess']) {
	$url = XOOPS_URL.'/read,'.$curr_category.','.($group).','.$artnum.','.($subject).'.'.$GLOBALS['xoopsModuleConfig']['seo_filetype'].'?';
...
$xoopsTpl->assign('newsgroup',"<a href='".XOOPS_URL."/modules/xpnews/indexing.php?category=$curr_category&group=$group'>$group</a>");
$xoopsTpl->assign('header',$artinfo);
$xoopsTpl->assign('xoops_pagetitle', $subject.' :: ' .$group);
