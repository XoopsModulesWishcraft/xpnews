...
$cache = "xpnews_index_array%%".$curr_category."_".$group.'_'.$news_server[$curr_category]."%%".$_GET['cursor'].'_'.$_GET['page'];
# ---------------------------------------------------------------------
...
if (!$read = XoopsCache::read($cache)){
	...
	XoopsCache::write($cache, array('code' => $code, 'count' => $count, 'curr_category' => $curr_category, 'group'=>$group,
...
if ($GLOBALS['xoopsModuleConfig']['htaccess']) {
	if ($cursor==0)
		$url = XOOPS_URL.'/indexing,'.$curr_category.','.$_GET['group'].'.'.$GLOBALS['xoopsModuleConfig']['seo_filetype'].'?';
	else
		$url = XOOPS_URL.'/indexing,'.$curr_category.','.$_GET['group'].','.$cursor.'.'.$GLOBALS['xoopsModuleConfig']['seo_filetype'].'?';
