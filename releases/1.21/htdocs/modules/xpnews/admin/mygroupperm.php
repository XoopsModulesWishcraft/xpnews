				...
				foreach ($item_ids as $item_id => $selected) {
					if ($selected == 1) {
						...
						if ($perm_data['parents'][$item_id] != '') {
							...
							foreach ($parent_ids as $pid) {
								if ($pid != 0 && !in_array($pid, array_keys($item_ids))) {
									...
									$msg[] = sprintf(constant($admin_lang."_PERMADDNG"), "<b>".$perm_name."</b>", "<b>".$perm_data['itemname'][$item_id]."</b>", "<b>".$group_list[$group_id]."</b>")." (".constant($admin_lang."_PERMADDNGP").")";
									continue 2;
								}
							}
						}
						...
						if (!$gperm_handler->insert($gperm)) {
							$msg[] = sprintf(constant($admin_lang."_PERMADDNG"), '<b>'.$perm_name.'</b>', '<b>'.$perm_data['itemname'][$item_id].'</b>', '<b>'.$group_list[$group_id].'</b>');
						} else {
							$msg[] = sprintf(constant($admin_lang."_PERMADDOK"), '<b>'.$perm_name.'</b>', '<b>'.$perm_data['itemname'][$item_id].'</b>', '<b>'.$group_list[$group_id].'</b>');
						}
						unset($gperm);
					}
				}
...
+ $msg[] = sprintf(constant($admin_lang."_PERMRESETNG"), $module->getVar('name').'('.$perm_name.')');
		}
	}
}
